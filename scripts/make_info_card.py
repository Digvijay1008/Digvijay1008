...
ROWS = [
    ("host",),
    ("kv", "Now", "AI Consultant @ Practus"),
    ("kv", "Exp", "1+ Year, 10+ Projects Deployed"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, SQL, JavaScript, Java"),
    ("kv", "AI / ML", "LangGraph, Agent Harness, OpenClaw, RAG"),
    ("kv", "Cloud/Web", "Azure AI Foundry, AWS Bedrock, React"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built AWS-secured RAG pipelines & AI Agents"),
    ("bul", "Conducted 8 AI Workshops for 300+ students"),
    ("bul", "260k+ Impressions & 148k+ Reach on LinkedIn"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">digvijay@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">digvijay</tspan><tspan fill="{MUTED}">@</tspan>'
